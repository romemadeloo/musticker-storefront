...
- Name: pricing/price-interpolation.spec.ts >> pricing interpolation kiss-cut-sticker >> MS-PRC-INT-kiss-cut-sticker area 146311 (283x517) interpolates between 144400 and 148225
- Location: tests/e2e/pricing/price-interpolation.spec.ts:103:7

# Error details

```
Error: kiss-cut-sticker area 146311 (283x517) qty 300 lower bound total should be the UNROUNDED 144400 row product: served pricing_id=48 "diecut/kisscut - 9/9/2026"

expect(received).toBeCloseTo(expected, precision)

Expected: 817700.0892
Received: 817708.3200000001

Expected precision:    4
Expected difference: < 0.00005
Received difference:   8.230800000019372
```

# Test source

```ts
  35  | test.describe.configure({ mode: 'parallel' });
  36  | 
  37  | // The API interpolates area from the MAGNITUDE of the gap between the two bound totals instead of its
  38  | // sign, so wherever a table's line total falls as area grows it overshoots and answers with a price
  39  | // above BOTH bounds -- which no linear interpolation between them can produce. Confirmed live on
  40  | // production 2026-08-25 and written up in tests/fixtures/pricing/README.md: transfer-sticker at area
