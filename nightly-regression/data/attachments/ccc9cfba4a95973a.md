...
- Name: validation/auth-validation.spec.ts >> auth validation and error handling >> registration form validates required fields without creating a user
- Location: tests/e2e/validation/auth-validation.spec.ts:16:3
...
Locator: getByRole('main').getByRole('heading', { name: '회원가입' })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('main').getByRole('heading', { name: '회원가입' })
...
+   "[error] Failed to load resource: the server responded with a status of 500 ()",
+   "[error] [nuxt] error caught during app initialization {error: true, url: /kr/auth/register, statusCode: 500, statusMessage: API Response Contract Violation: Expected ApiResponse structure., message: API Response Contract Violation: Expected ApiResponse structure.}",
+   "[error] Failed to load resource: the server responded with a status of 500 ()",
...
- Array []
+ Array [
+   "500 https://www.musticker.com/kr/auth/register",
+   "500 https://api.musticker.com/index.php/sys/kr/navigation/categories",
+   "500 https://api.musticker.com/index.php/sys/kr/user/me?t=1785874045337",
+   "500 https://api.musticker.com/index.php/sys/kr/seo?page=%2Fkr%2Fauth%2Fregister",
+ ]
```
